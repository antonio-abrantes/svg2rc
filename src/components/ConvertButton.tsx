...
export function ConvertButton({ onClick, disabled, variant = "floating" }: ConvertButtonProps) {
  ...
  return (
    <button
      ...
      aria-label={applyAria}
      title={applyAria}
      className={`group absolute left-1/2 top-1/2 z-10 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border shadow-lg transition-all ${
        disabled
